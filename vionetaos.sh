...
readonly CASA_VERSION_URL="https://api.vionetaos.io/vionetaos-api/version"
...
Check_Exist() {
    ...
    if [[ -f "/usr/lib/systemd/system/vionetaos.service" ]]; then
        ${sudo_cmd} rm -rf /usr/lib/systemd/system/vionetaos.service
    fi

    if [[ -f "/lib/systemd/system/vionetaos.service" ]]; then
    ...
    if [[ -f "/vionetaos/server/conf/conf.ini" ]]; then
        ${sudo_cmd} cp -rf /vionetaos/server/conf/conf.ini ${CASA_CONF_PATH}
        exist_file /vionetaos/server/conf/*.json
        value=$?
        if [ $value -eq 1 ]; then
            ${sudo_cmd} cp -rf /vionetaos/server/conf/*.json ${CASA_USER_CONF_PATH}
        fi
    fi

    if [[ -d "/vionetaos/server/db" ]]; then
        ${sudo_cmd} cp -rf /vionetaos/server/db/* ${CASA_DB_PATH}
    fi
...
#Download VionetaOS Package
Download_VionetaOS() {
    Show 2 "Downloading VionetaOS for ${Target_OS}/${Target_Arch}..."
    Net_Getter="curl -fsSLk"
    Casa_Package="${Target_OS}-${Target_Arch}-vionetaos${CASA_PACKAGE_EXT}"
    if [[ ! -n "$version" ]]; then
    ...
    *.tar.gz) ${sudo_cmd} tar -xzf "$PREFIX/tmp/${Casa_Package}" -C "$PREFIX/tmp/" ;;
    ...
    ${sudo_cmd} ${Net_Getter} "$CASA_UNINSTALL_URL" >"$PREFIX/tmp/vionetaos-uninstall"
...
#Install Addons
Install_Addons() {
    Show 2 "Installing VionetaOS Addons"
...
Install_VionetaOS() {
    ...
    if [[ ! -x "$(command -v ${CASA_BIN})" ]]; then
        ...
        Show 0 "VionetaOS Successfully installed."
    fi
}
...
Generate_Service() {
    if [ -f ${CASA_SERVICE_PATH} ]; then
        Show 2 "Try stop VionetaOS system service."
        # Stop before generation
        if [[ $(systemctl is-active ${CASA_BIN}) == "active" ]]; then
            ${sudo_cmd} systemctl stop ${CASA_BIN}
        fi
    fi
    Show 2 "Create system service for VionetaOS."

    ${sudo_cmd} tee ${CASA_SERVICE_PATH} >/dev/null <<EOF
				[Unit]
				Description=VionetaOS Service
...
EOF
    Show 0 "VionetaOS service Successfully created."
}
...
Start_VionetaOS() {
    ...
    Show 2 "Create a system startup service for VionetaOS."
    $sudo_cmd systemctl daemon-reload
    $sudo_cmd systemctl enable ${CASA_BIN}

    Show 2 "Start VionetaOS service."
...
# Step 9: Install VionetaOS
Install_VionetaOS

# Step 10: Generate_Service
Generate_Service

# Step 11: Start VionetaOS
Start_VionetaOS
Clean_Temp_Files
