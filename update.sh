#!/usr/bin/bash
#
#           VionetaOS Update Script v0.1.0-1#
...
# 
#   This script update your VionetaOS.
#   Usage:
#
#   	$ wget -qO- https://get.vionetaos.io/update | bash
#   	  or
#   	$ curl -fsSL https://get.vionetaos.io/update | bash
#
...
readonly CASA_UNINSTALL_URL="https://get.vionetaos.io/uninstall/v0.1.0"
...
# CASAOS VARIABLES
TARGET_ARCH=""
TMP_ROOT=/tmp/vionetaos-installer
CASA_DOWNLOAD_DOMAIN="https://github.com/"


# PACKAGE LIST OF CASAOS
CASA_SERVICES=(
    "vionetaos-gateway.service"
"vionetaos-message-bus.service"
"vionetaos-user-service.service"
"vionetaos-local-storage.service"
"vionetaos-app-management.service"
"rclone.service"
"vionetaos.service"  # must be the last one so update from UI can work 
)
...
upgradePath="/var/log/vionetaos"
upgradeFile="/var/log/vionetaos/upgrade.log"
...
Show() {
    # OK
    if (($1 == 0)); then
    	echo -e "- OK $2" | ${sudo_cmd} tee -a /var/log/vionetaos/upgrade.log
    # FAILED
    elif (($1 == 1)); then
     	echo -e "- FAILED $2" | ${sudo_cmd} tee -a /var/log/vionetaos/upgrade.log
	exit 1
    # INFO
    elif (($1 == 2)); then
    	echo -e "- INFO $2" | ${sudo_cmd} tee -a /var/log/vionetaos/upgrade.log
    # NOTICE
    elif (($1 == 3)); then
    	echo -e "- NOTICE $2" | ${sudo_cmd} tee -a /var/log/vionetaos/upgrade.log
    fi
}
...
Get_Download_Url_Domain() {
    ...
    if [[ "${REGION}" = "China" ]] || [[ "${REGION}" = "CN" ]]; then
        CASA_DOWNLOAD_DOMAIN="https://vionetaos.oss-cn-shanghai.aliyuncs.com/"
    fi
}
...
Check_Arch() {
    ...
    *)
    ...
    CASA_PACKAGES=(
        "${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-Gateway/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-gateway-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-MessageBus/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-message-bus-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-UserService/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-user-service-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-LocalStorage/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-local-storage-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-AppManagement/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-app-management-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-CLI/releases/download/v0.1.0/linux-${TARGET_ARCH}-vionetaos-cli-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-UI/releases/download/v0.1.0/linux-all-vionetaos-v0.1.0.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}Vioneta/VionetaOS-AppStore/releases/download/v0.1.0/linux-all-appstore-v0.1.0.tar.gz" 
    )
}
...
Install_rclone_from_source() {
  ...
  if [[ "${REGION}" = "China" ]] || [[ "${REGION}" = "CN" ]]; then
    sed -i 's/downloads.rclone.org/vionetaos.oss-cn-shanghai.aliyuncs.com/g' ./install.sh
  else
    sed -i 's/downloads.rclone.org/get.vionetaos.io/g' ./install.sh
  fi
...
Configuration_Addons() {
    Show 2 "Configuration VionetaOS Addons"
    ...
    if [[ -f "${PREFIX}${UDEVIL_CONF_PATH}" ]]; then
        ...
        # Revert previous VionetaOS udevil configuration
        #shellcheck disable=SC2016
...
# Download And Install VionetaOS
DownloadAndInstallVionetaOS() {
    ...
    ${sudo_cmd} cp -rf "${SYSROOT_DIR}"/* / >> /dev/null || Show 1 "Failed to install VionetaOS"
    ...
    # Reset Permissions
    UI_EVENTS_REG_SCRIPT=/etc/vionetaos/start.d/register-ui-events.sh
    if [[ -f ${UI_EVENTS_REG_SCRIPT} ]]; then
        ${sudo_cmd} chmod +x $UI_EVENTS_REG_SCRIPT
    fi
    
    # Modify app store configuration
    sed -i "/ServerAPI/d" "$PREFIX/etc/vionetaos/app-management.conf"
    sed -i "/ServerApi/d" "$PREFIX/etc/vionetaos/app-management.conf"
    sed -i '/appstore/d' "$PREFIX/etc/vionetaos/app-management.conf"
    sed -i "/server/aappstore = ${CASA_DOWNLOAD_DOMAIN}Vioneta/_appstore/archive/refs/heads/main.zip" "$PREFIX/etc/vionetaos/app-management.conf"
    
    #Download Uninstall Script
    if [[ -f ${PREFIX}/tmp/vionetaos-uninstall ]]; then
        ${sudo_cmd} rm -rf "${PREFIX}/tmp/vionetaos-uninstall"
    fi
    ${sudo_cmd} curl -fsSLk "$CASA_UNINSTALL_URL" >"${PREFIX}/tmp/vionetaos-uninstall"
    ${sudo_cmd} cp -rvf "${PREFIX}/tmp/vionetaos-uninstall" $CASA_UNINSTALL_PATH || {
    ...
    Show 0 "VionetaOS upgrade successfully"
    for SERVICE in "${CASA_SERVICES[@]}"; do
...
# Step 4: Download And Install VionetaOS
DownloadAndInstallVionetaOS
