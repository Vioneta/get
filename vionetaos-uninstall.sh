...
#
#   This script uninstalls VionetaOS to your path.
#   Usage:
#
...
readonly TITLE="VionetaOS Uninstaller"

readonly CASA_PATH=/vionetaos/server

readonly casa_bin="vionetaos"
install_path="/usr/local/bin"
service_path=/usr/lib/systemd/system/vionetaos.service
if [ ! -d "/usr/lib/systemd/system" ]; then
    service_path=/lib/systemd/system/vionetaos.service
    if [ ! -d "/lib/systemd/system" ]; then
        service_path=/etc/systemd/system/vionetaos.service
    fi
fi
...
uninstall_vionetaos() {
    ...
    #stop and remove vionetaos service
    remove_serveice $service_path $install_path/vionetaos

    #remove casa containers
    if [[ $REMOVE_CASAOS_CONTAINERS = true ]]; then
        #stop all vionetaos‘s containers
...
#######################################
# Custom remove vionetaos function
# Globals:
#   None
# Arguments:
#   None
...
#   None
# Arguments:
#   $1 0:service path   1:vionetaos path
# Returns:
#   None
#######################################
remove_serveice() {
    ((EUID)) && sudo_cmd="sudo"
    $sudo_cmd systemctl disable vionetaos
    if [ -f $service_path ]; then
        show 2 "Try stop VionetaOS system service."
        $sudo_cmd systemctl stop vionetaos.service # Stop before generation
    fi
    $sudo_cmd rm $1
    $sudo_cmd rm $2
}
...
if (whiptail --title "${TITLE}" --yesno --defaultno "Do you want uninstall docker?" 10 60); then
    ...
    if (whiptail --title "${TITLE}" --yesno --defaultno "Do you want remove all containers of VionetaOS?" 10 60); then
        REMOVE_CASAOS_CONTAINERS=true
    fi
fi

if (whiptail --title "${TITLE}" --yesno --defaultno "Do you want remove all files of VionetaOS?" 10 60); then
