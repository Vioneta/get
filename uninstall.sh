...
# New Casa Files
readonly MANIFEST=/var/lib/vionetaos/manifest
readonly CASA_CONF_PATH_OLD=/etc/vionetaos.conf
readonly CASA_CONF_PATH=/etc/vionetaos
readonly CASA_RUN_PATH=/var/run/vionetaos
readonly CASA_USER_FILES=/var/lib/vionetaos
readonly CASA_LOGS_PATH=/var/log/vionetaos
readonly CASA_HELPER_PATH=/usr/share/vionetaos
...
Uninstall_Casaos() {
    ...
    # Old Casa Files
    if [[ -d ${CASA_PATH} ]]; then
        ${sudo_cmd} rm -rf ${CASA_PATH} || Show 1 "Failed to delete VionetaOS files."
    fi
    ...
    if [[ -f ${CASA_BIN} ]]; then
        ${sudo_cmd} rm -rf ${CASA_BIN} || Show 1 "Failed to delete VionetaOS exec file."
    fi
...
# Check user
if [ "$(id -u)" -ne 0 ];then
    Show 1 "Please execute with a root user, or use ${aCOLOUR[4]}sudo vionetaos-uninstall${COLOUR_RESET}."
    exit 1
fi
